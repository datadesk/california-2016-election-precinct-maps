...
with open(sys.argv[1].replace('pdf','txt')) as file:
	...
	resultstring = re.sub(r'()(.*\n)+?(\d{6} )',r'\3','%s' % file.read(),flags=re.MULTILINE)
	print resultstring
	...
	resultstring = re.sub(r'(\d{6})( .+?,)',r'\1,',resultstring)
	...
	# column_names = ['pct16','reg','ballots','turnout','pres_stein','pres_clinton','pres_lariva','pres_trump','pres_johnson','ussenate_sanchez','ussenate_harris']
