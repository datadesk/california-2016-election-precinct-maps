...
with open(sys.argv[1].replace('pdf','txt')) as file:
	...
	# out.write('pct16,total,pres_trump,trump_pct,pres_johnson,johnson_pct,pres_other,pres_other_pct\n')
	...
	resultstring = re.sub(r'(.*\n){12}( *(Jurisdiction|To.{,5}|Tola!| Tota l|Mail Ballot|Provisional|\d{1,2}[A-Z  -]+)?)',r'\2','%s' % file.read(),flags=re.MULTILINE)
	resultstring = re.sub(r'(?:\n\s{3,})(To.{1,5})',r'\nTotal  ',resultstring,flags=re.MULTILINE)
	# resultstring = re.sub(r'(?:\n\s{3,})(\w{3} \d{3}-\d{2}|MB\d{2})',r'\n\1',resultstring,flags=re.MULTILINE)
	print resultstring
	results = [re.findall(r'(?: *)(\d{1,2}[A-Z  -]+)(?:\n(.*\n){2} *)(To\D{,5}.+)',resultstring,flags=re.MULTILINE)]
	# print results
